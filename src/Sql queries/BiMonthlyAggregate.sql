...
DECLARE @siteType NVARCHAR(50) = 'NCDC-Weather'--'USBR-Reservoir'--'USGS-Streamflow'--
...
From(
	SELECT 
		...
		-- If streamflow. Multiply value by number of seconds in interval, and divide by number of feet in an acre(43560) to get acre feet 
		CASE 
		...
		END AS MaxDataValue, 
		CASE
			WHEN DataValue = Data.NoDataValue
			THEN	NULL
			WHEN @siteType LIKE '%USGS%' AND MonthPart = 1
	...
	FROM  
      ...
      WHEN MAX(CASE
		...
		END AS EndDate,
					...
					WHERE --DV.DataValue <> V.NoDataValue 
						--AND 
						DV.LocalDateTime Between(-- case statement checks to see if there is a valid enddate from the series catalog entry
						CASE
			...
			CASE
				WHEN a.DataValue= a.NoDV OR b.DataValue =b.NoDV THEN a.NoDV	
				 ...
				 ELSE
					a.NoDV
			END AS Value, 
			--ABS(DATEDIFF(DAY, a.LocalDateTime, b.LocalDateTime))AS diff,
			--a.DataValue AS aval,
			--a.LocalDateTime AS atime,
			--a.RowNum AS anum,
			--b.DataValue AS bval,
			--b.LocalDateTime AS btime,
			--b.RowNum AS bnum,
			a.SiteID,
			a.MonthPart, 
			(CASE
				...
				SELECT 
					...
					WHERE  VariableID = (SELECT MAX(VariableID) FROM #temp1 WHERE AggregateMethod <> 'Delta') AND 
						LocalDateTime = (SELECT MAX(LocalDateTime) 
											...
											'Data Source=wasser.uwrl.usu.edu;Initial Catalog = SummaryTest;User ID=NIDIS;Password=N1d1s!'
											).SummaryTest.dbo.DataValues 
...
WHERE #temp1.LocalDateTime = Data.LocalDateTime 
		...
		--#temp1.VariableID= 23--VariableId is from #temp1
